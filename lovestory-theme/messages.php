...
$active_partner_id = 0;
$current_user_blocks_partner = false;
$partner_blocks_current = false;
$is_conversation_blocked = false;
$can_manage_partner_block = false;

if ($active_conversation) {
    $active_partner_id = ((int) $active_conversation->user_one_id === (int) $current_user_id)
        ? (int) $active_conversation->user_two_id
        : (int) $active_conversation->user_one_id;

    if (
        $active_partner_id &&
        function_exists('dating_user_has_blocked_user')
    ) {
        $current_user_blocks_partner = dating_user_has_blocked_user($current_user_id, $active_partner_id);
        $partner_blocks_current = dating_user_has_blocked_user($active_partner_id, $current_user_id);
        $is_conversation_blocked = $current_user_blocks_partner || $partner_blocks_current;
    }

    $can_manage_partner_block = $active_partner_id > 0 && !$is_support_conversation;
}
...
>
  ...
  data-partner-id="<?php echo esc_attr($active_partner_id); ?>"
  data-current-user-blocks-partner="<?php echo $current_user_blocks_partner ? '1' : '0'; ?>"
  data-partner-blocks-current="<?php echo $partner_blocks_current ? '1' : '0'; ?>"
>
          <?php if ($active_conversation) : ?>
            ...
            <header class="messages__chat-header">
              ...
              <div class="messages__chat-actions">
                ...
                <?php if ($can_manage_partner_block) : ?>
                  <button
                    class="messages__block-button <?php echo $current_user_blocks_partner ? 'messages__block-button--unblock' : ''; ?>"
                    type="button"
                    data-user-block-action="<?php echo $current_user_blocks_partner ? 'unblock' : 'block'; ?>"
                    data-blocked-user-id="<?php echo esc_attr($active_partner_id); ?>"
                  >
                    <?php echo $current_user_blocks_partner ? 'Разблокировать' : 'Заблокировать'; ?>
                  </button>
                <?php endif; ?>
              </div>
            </header>

            <?php if ($is_conversation_blocked) : ?>
              <div class="messages__block-notice" role="note" data-block-notice>
                <?php if ($current_user_blocks_partner && $partner_blocks_current) : ?>
                  Между вами и этим пользователем установлена взаимная блокировка. Отправка сообщений недоступна.
                <?php elseif ($current_user_blocks_partner) : ?>
                  Вы заблокировали этого пользователя. Чтобы продолжить переписку, сначала разблокируйте его.
                <?php else : ?>
                  Этот пользователь ограничил переписку с вами. Отправка сообщений недоступна.
                <?php endif; ?>
              </div>
            <?php endif; ?>
...
<?php endforeach; ?>
            ...
            <?php else : ?>
              <div class="messages__form messages__form--blocked" data-message-form-blocked>
                <p class="messages__form-message messages__form-message--blocked">
                  Отправка сообщений в этом диалоге сейчас недоступна.
                </p>
              </div>
            <?php endif; ?>
